  ...
  describe('Basic test info', function () {
    beforeEach(function () {
      browser.addMockModule('initialGetMock', function () {
        angular.module('initialGetMock', ['ngMockE2E']).run(function ($httpBackend) {
    ...
    it('should hold the artifact id, group id and version of the artifact after selecting an artifact', function () {
      ...
      expect(mainPage.getElementTextById('artifact-id')).toEqual('wix-public-html-renderer-webapp');
      expect(mainPage.getElementTextById('group-id')).toEqual('com.wixpress');
      expect(mainPage.getElementTextById('artifact-version')).toEqual('2.487.0');
    });
  });
